...
ShaderES3::ShaderES3(ShaderData& data) {
    ...
    glShaderSource(VertexShader, 1, &data.vertexShaderSrc, NULL);
    glCompileShader(VertexShader);

    GLint vertex_shader_status;
    glGetShaderiv(VertexShader, GL_COMPILE_STATUS, &vertex_shader_status);

    char buff[512];

    if(!vertex_shader_status) {
        glGetShaderInfoLog(VertexShader, 512, nullptr, buff);
        printf("%s\n", buff);
    }

    char vertexLogBuffer[512];
    GLuint FragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(FragmentShader, 1, &data.pixelShaderSrc, NULL);
    glCompileShader(FragmentShader);

    GLint fragment_shader_status;
    glGetShaderiv(FragmentShader, GL_COMPILE_STATUS, &fragment_shader_status);

    if(!fragment_shader_status) {
        glGetShaderInfoLog(FragmentShader, 512, nullptr, buff);
        printf("%s\n", buff);
    }
