#ifndef NODES_DIFFERENT_H
#define NODES_DIFFERENT_H
...
math_shape_desc_t* parse_different_node(const rapidxml::xml_node<> *node, Font* font) {
    ...
    for(const rapidxml::xml_node<>* n = first_node->next_sibling(); n; n = n->next_sibling()) {
        ...
        concat_math_shape_descs(tmp, add_shape_desc, tmp->p1_x - add_shape_desc->p0_x, tmp->y - add_shape_desc->y);
        concat_math_shape_descs(tmp, end_shaped_desc, tmp->p1_x - end_shaped_desc->p0_x, tmp->y - end_shaped_desc->y);
    }
    return tmp;
}
